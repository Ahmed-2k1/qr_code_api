...
class QRCodeRequest(BaseModel):
    ...
    fill_color: str = Field(
        default="red", description="Color of the QR code.", example="black")
    back_color: str = Field(
        default="white", description="Background color of the QR code.", example="yellow")
    size: conint(ge=1, le=40) = Field( # type: ignore
        default=10, description="Size of the QR code from 1 to 40.", example=20)  # type: ignore
...
class QRCodeResponse(BaseModel):
    ...
    links: List[Link] = Field(
        default=[], description="HATEOAS links related to the QR code.")
